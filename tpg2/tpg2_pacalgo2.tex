...
\subsection{Consideraciones TP1}
...
\subsection{Consideraciones TP2}

    \begin{itemize}
       \item Puede haber más de un jugador en una posición del ranking
       \item Si el jugador actual está en el primer puesto, el contrincante a superar puede ser él mismo
       \item No todos los jugadores juegan el mismo mapa
    \end{itemize}
...
\begin{tad}{\tadNombre{Pacalgo2}}
    ...
    \tadUsa {Mapa}
    \tadGeneros{pacalgo2}
    \tadExporta{pacalgo2, observadores, generadores, direccionesPosibles, perdió?, ganó?}
...
\begin{tad}{\tadNombre{Ranking}}
    ...
    \tadUsa{Nat, persona}
    \tadGeneros{ranking}
    \tadExporta{observadores, generadores, contrincante}
    \tadIgualdadObservacional{r_1}{r_2}{ranking}{ver($r_1$)$\igobs$ver($r_2$)}
    ...
    \tadAlinearFunciones{cargarPuntaje}{ranking, persona, nat}
        \tadOperacion{iniciarRanking}{}{ranking}{}
    ...
    \tadAlinearFunciones{laOtraFunciónQueHayQueHacer}{ranking\ $r$, conj(persona)\ $nombres$, persona\ $p$}
        ...
        \tadOperacion{elMaximoEntreMenores\footnotemark[1]}{conj($nat$)\ $conjunto$, nat\ $p$}{nat}{}
        ...
        \tadOperacion{máximoDeConjunto}{conj(nat)\ $C$}{nat}{$\lnot\emptyset$?($C$)}

    \footnotetext[1]{Si no hay un menor que $p$ en el $conjunto$, devuelve $p$}

    \tadAxiomas
    \tadAlinearAxiomas{jugadoresConPuntajeIgual($r$, $nombres$, $puntaje$)}
        \tadAxioma{ver(iniciarRanking())}{vacío}
        ...
        \tadAxioma{elMaximoEntreMenores($puntajes$, $p$)}{\IF $\emptyset$?(menoresQueP($puntajes$, $p$)) THEN $p$ ELSE máximoDeConjunto(menoresQueP($puntajes$, $p$)) FI}
        ...
        \tadAxioma{máximoDeConjunto($C$)}{\IF $\emptyset$?(sinUno($C$)) THEN dameUno($C$) ELSE $\max$(dameUno($C$), máximoDeConjunto(sinUno($C$))) FI}    
        \tadAxioma{contrincante($r$,$j$)}{(dameUno(superioresInmediatos($r$,$j$)), obtener(ver($r$), dameUno(superioresInmediatos($r$,$j$))))}
...
\begin{tad}{\tadNombre{Fichín}}
    ...
    \tadUsa{Ranking, Pacalgo2}
    \tadGeneros{fichín}
    \tadExporta{observadores, generadores, otras operaciones}
    \tadIgualdadObservacional{f_1}{f_2}{fichín}{conocerRanking($f_1$)$\igobs$conocerRanking($f_2$) $\wedge$ \\
            estáLibre?($f_1$)$\igobs$ estáLibre?($f_2$) $\wedge$ \\
            ($\neg$estáLibre?($f_1$) $\implies_L$ \\
                verJuegoActual($f_1$)$\igobs$verJuegoActual($f_2$) $\wedge$ \\
                jugadorActual($f_1$)$\igobs$jugadorActual($f_2$))}

    \tadObservadores
    \tadAlinearFunciones{conocerRanking}{fichín\ $f$}
        \tadOperacion{conocerRanking}{fichín}{ranking}{}
        \tadOperacion{verJuegoActual}{fichín\ $f$}{pacalgo2}{$\neg$estáLibre?($f$)}
        \tadOperacion{jugadorActual}{fichín\ $f$}{persona}{$\neg$estáLibre?($f$)}

    \tadGeneradores
    \tadAlinearFunciones{palancaIzquierda}{fichín\ $f$, pacalgo2\ $p$, persona\ $j$}
        \tadOperacion{nuevoFichin}{}{fichín}{}
        \tadOperacion{nuevaPartida}{fichín\ $f$, pacalgo2\ $p$, persona\ $j$}{fichín}{estáLibre?($f$)}
        \tadOperacion{palancaArriba}{fichín\ $f$}{fichín}{$\neg$estáLibre?($f$)$\wedge_L$\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
            (posiciónActual(verJuegoActual($f$)) + $\langle 0,1\rangle$)\ $\in$\ direccionesPosibles(verJuegoActual($f$))}
        \tadOperacion{palancaAbajo}{fichín\ $f$}{fichín}{$\neg$estáLibre?($f$)$\wedge_L$\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
            (posiciónActual(verJuegoActual($f$)) + $\langle 0,-1\rangle$)\ $\in$\ direccionesPosibles(verJuegoActual($f$))}
        \tadOperacion{palancaDerecha}{fichín\ $f$}{fichín}{$\neg$estáLibre?($f$)$\wedge_L$\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
            (posiciónActual(verJuegoActual($f$)) + $\langle 1,0\rangle$)\ $\in$\ direccionesPosibles(verJuegoActual($f$))}
        \tadOperacion{palancaIzquierda}{fichín\ $f$}{fichín}{$\neg$estáLibre?($f$)$\wedge_L$\qquad\qquad\qquad\qquad\qquad\qquad\qquad\\
            (posiciónActual(verJuegoActual($f$)) + $\langle -1,0\rangle$)\ $\in$\ direccionesPosibles(verJuegoActual($f$))}

    \tadOtrasOperaciones
    \tadAlinearFunciones{puntajeDeJugadorActual}{fichín\ $f$}
        \tadOperacion{puntajeDeJugadorActual}{fichín\ $f$}{nat}{$\neg$estálibre?($f$)\ $\wedge_L$jugadorActual($f$)$\in$claves(ver(conocerRanking($f$)))}
        \tadOperacion{verContrincante}{fichín\ $f$}{tupla(persona, nat)}{$\neg$estáLibre?($f$)$\wedge_L$jugadorActual($f$)$\in$claves(ver(conocerRanking($f$)))}
        \tadOperacion{estáLibre?}{fichín}{Bool}{}
        \tadOperacion{puntajeParaMejorar}{fichín\ $f$}{nat}{$\neg$estálibre?($f$)\ $\wedge_L$jugadorActual($f$)$\in$claves(ver(conocerRanking($f$)))}

    \tadAxiomas
    \tadAlinearAxiomas{verJuegoActual(nuevaPartida($f$, $p$, $j$))}
        \tadAxioma{estáLibre?(nuevoFichin())}{True}
        \tadAxioma{estáLibre?(nuevaPartida($f$,$p$,$j$))}{False}
        \tadAxioma{estáLibre?(palancaArriba($f$))}{ganó?(arriba(verJuegoActual($f$)))$\vee$perdió?(arriba(verJuegoActual($f$)))}
        \tadAxioma{estáLibre?(palancaAbajo($f$))}{ganó?(abajo(verJuegoActual($f$)))$\vee$perdió?(abajo(verJuegoActual($f$)))}
        \tadAxioma{estáLibre?(palancaDerecha($f$))}{ganó?(derecha(verJuegoActual($f$)))$\vee$perdió?(derecha(verJuegoActual($f$)))}
        \tadAxioma{estáLibre?(palancaIzquierda($f$))}{ganó?(izquierda(verJuegoActual($f$)))$\vee$perdió?(izquierda(verJuegoActual($f$)))}
        \tadAxioma{jugadorActual(nuevaPartida($f$,$p$,$j$))}{$j$}
        \tadAxioma{jugadorActual(palancaArriba($f$))}{jugadorActual($f$)}
        \tadAxioma{jugadorActual(palancaAbajo($f$))}{jugadorActual($f$)}
        \tadAxioma{jugadorActual(palancaDerecha($f$))}{jugadorActual($f$)}
        \tadAxioma{jugadorActual(palancaIzquierda($f$))}{jugadorActual($f$)}
        \tadAxioma{conocerRanking(nuevoFichin())}{iniciarRanking()}
        ...
        \tadAxioma{puntajeParaMejorar($f$)}{pred($\pi_2$(verContrincante($f$)))}
\end{tad}
