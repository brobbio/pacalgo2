...
\begin{tad}{\tadNombre{Fichín}}
    ...
    \tadGeneradores
        ...
        \tadOperacion{palancaArriba}{fichin\ $f$}{fichin}{$\neg$estáLibre?($f$)}
        \tadOperacion{palancaAbajo}{fichin\ $f$}{fichin}{$\neg$estáLibre?($f$)}
        \tadOperacion{palancaDerecha}{fichin\ $f$}{fichin}{$\neg$estáLibre?($f$)}
        \tadOperacion{palancaIzquierda}{fichin\ $f$}{fichin}{$\neg$estáLibre?($f$)}

    \tadOtrasOperaciones
        \tadOperacion{verContrincante}{fichin\ $f$}{persona}{jugadorActual($f$)$\in$claves(ver(conocerRanking($f$)))}
