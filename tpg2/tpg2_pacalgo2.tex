...
\begin{tad}{\tadNombre{Ranking}}
    ...
    \tadAlinearFunciones{laOtraFunciónQueHayQueHacer}{ranking\ $r$, conj($string$)\ $claves$, string\ $p$}
        ...
        \tadOperacion{superioresInmediatos}{ranking\ $r$, string\ $j$}{conj(string)}{$\neg\emptyset$($claves$) $\wedge$ def?($j$, ver($r$))}
        \tadOperacion{elMaximoEntreMenores}{conj($nat$)\ $puntajes$, nat\ $p$}{nat}{}
        \tadOperacion{todosLosPuntajes}{ranking}{conj(nat)}{}
        \tadOperacion{jugadoresConPuntajeIgual}{ranking, conj(string), nat}{conj(string)}{}
    ...
    \tadAlinearAxiomas{superioresInmediatos($ranking$, $claves$, $jugador$)}
        ...
        \tadAxioma{elMaximoEntreMenores($puntajes$, $p$)}{\IF $\emptyset$?(sinUno($puntajes$)) THEN 
                $p$ 
            ELSE 
                {\IF dameUno($puntajes$) $\geq$ $p$ THEN
                    elMaximoEntreMenores(sinUno($puntajes$),$p$)
                ELSE
                    max\{dameUno($puntajes$), elMaximoEntreMenores(sinUno($puntajes$),$p$)\}
                FI}
            FI}
