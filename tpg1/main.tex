...
\begin{tad}{\tadNombre{Mapa}} 
    ...
    \tadIgualdadObservacional{m_1}{m_2}{mapa}{fantasmas($m_1$)$\igobs$fantasmas($m_2$)$\ \wedge$\\
        paredes($m_1$)$\igobs$paredes($m_2$)\ $\wedge$\\
    ...
    \tadAxiomas[]
        \tadAxioma{fantasmas(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$fs$}
        \tadAxioma{paredes(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$ps$}
        \tadAxioma{dimensiones(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$dimensi\acute{o}n$}
        \tadAxioma{casilleroInicial(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$inicio$}
        \tadAxioma{casilleroDeLlegada(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$fin$}
        \tadAxioma{vertice(nuevoMapa($dimensi\acute{o}n$, $vertice$, $inicio$, $fin$, $fs$, $ps$))}{$vertice$}
...
\tadAlinearAxiomas{trayectoria(InicializarJuego $m$)}
    ...
    \tadAxioma{perdió?($p$)}{ $\emptyset$?(fantasmas(verMapa($p$)) $\cap$ aDistanciaMenosDeN(posicionActual($p$), 3)) }
