...
\begin{tad}{\tadNombre{Mapa}} 
    ...
    \tadAxiomas[]
        ...
        \tadAxioma{casilleroDeLlegada(nuevoMapa($dimension$, $inicio$, $fin$, $fs$, $ps$))}{$fin$}
...
\tadAlinearAxiomas{trayectoria(InicializarJuego $m$)}
    ...
    \tadAxioma{perdió?($p$)}{$\exists$ (f $\in$ fantasmas(verMapa($p$)))\\ (f $\in$ aDistanciaMenosDeN(posiciónActual($p$),3))}
